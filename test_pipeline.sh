...
python -m camp3d tools write-addon ~/.config/blender/4.2/scripts/addons/

# Install example materials and scenes
echo "Step 3: Installing example materials and scenes..."
python -m camp3d tools install-materials
python -m camp3d tools install-examples
...
if command -v blender &> /dev/null; then
    ...
    python -m camp3d create-blend --create-new-scene \
...
if [ -d "./helios/output" ]; then
    ...
    python -m camp3d postprocess-ml --input-root ./helios/output --output-root ./ml_data_test --tile-size 50.0
else
